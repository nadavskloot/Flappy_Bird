...
# create a time frame that works?
# Make bird move propotional to pipe movement. Move 32 up or down.
...
class Bird(pygame.sprite.Sprite):
    ...
    def update(self, delta_frames=1):
        ...
        delta_frames: The number of frames elapsed since this method was
            last called.
        """
        # if self.msec_to_climb > 0: # NADAV: Make bird go up 32 and right 32!!!!!!!
        ...
        if self.jumping:
            self.y -= Bird.SINK_SPEED * frames_to_msec(delta_frames) + 0.4
        else:
            self.y += Bird.SINK_SPEED * frames_to_msec(delta_frames)
...
def main():
    ...
    while not done:
        ...
        if not (paused or frame_clock % msec_to_frames(PipePair.ADD_INTERVAL)):
            ...
            birdY = 16 - round(bird.y / 32)
            birdX = round(bird.x / 32)
            pipeX = round(pp.x / 32)
            pipeY = round(pp.bottom_height_px / 32)
            pipeY2 = 16- round(pp.top_height_px/ 32)
            print(birdX, birdY)
            print(pipeX, pipeY, pipeY2)
            # time.sleep(10) 

            actionList = PipeMaze_ForPygame.runQLearning(birdX, birdY, pipeX, pipeY)
            listPos = 0

        if count % 10 == 0 and len(actionList) != 0:
            # print(listPos)
            if actionList[listPos] == 1:
                bird.jumping = True
            else:
                bird.jumping = False
            listPos += 1
