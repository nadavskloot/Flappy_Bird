...
def setUpOnePipe(rewardGrid, x, y):
    ...
    for i in range(height):
        if (i != y) and (i != y+1) and (i != y+2) and (i != y+3):
            rewardGrid[(height-1)-i][x] = -1
            rewardGrid[(height-1)-i][x+1] = -1

def setUpPipes(pipeX, PipeY):
    global height, length
    rewardGrid = np.zeros((height,length))
    Q = setUpQ()

    setUpOnePipe(rewardGrid, pipeX, PipeY + 2)
...
def qLearning(gamma, epsilon, alpha, n, birdX, birdY, pipeX, pipeY):
    global length, height
    np.random.seed(int(time.time()))
    episodes = 0

    Q, rewardGrid = setUpPipes(pipeX, pipeY)
...
def runQLearning(birdX, birdY, pipeX, pipeY):
    global height, length
    gamma = 0.9
    epsilon = 0.7
    alpha = 0.1
    Q, rewardGrid = qLearning(gamma, epsilon, alpha, 100000, birdX, birdY, pipeX, pipeY)
